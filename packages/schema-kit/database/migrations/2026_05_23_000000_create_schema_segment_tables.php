...
return new class extends Migration
{
    public function up(): void
    {
        Schema::create('schema_segment_groups', function (Blueprint $table) {
            ...
            $table->timestamps();
            $table->unique(['schema_definition_id', 'slug'], 'ssg_schema_slug_unique');
            $table->index(['scope_type', 'scope_id'], 'ssg_scope_index');
        });
        ...
        Schema::create('schema_segments', function (Blueprint $table) {
            ...
            $table->boolean('is_active')->default(true);
            $table->timestamps();
            $table->unique(['schema_definition_id', 'slug'], 'ss_schema_slug_unique');
            $table->index(['scope_type', 'scope_id'], 'ss_scope_index');
        });
        ...
        Schema::create('schema_segment_members', function (Blueprint $table) {
            ...
            $table->timestamps();
            $table->unique(['schema_segment_id', 'entity_type', 'entity_id'], 'ssm_segment_entity_unique');
            $table->index(['entity_type', 'entity_id'], 'ssm_entity_index');
        });
    }
