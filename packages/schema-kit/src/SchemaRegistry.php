...
final class SchemaRegistry
{
    ...
    public function resolveView(string $entityName, string $viewName, ?FacetResolutionContext $context = null): ResolvedViewDefinition
    {
        $entity = $this->entity($entityName);
        $view = $entity->requireView($viewName);
        $facets = $this->resolveFacets($entityName, $view->getFacetNames(), $context);
     ...
     */
    public function resolveFacets(string $entityName, ?array $facetNames = null, ?FacetResolutionContext $context = null): array
    {
        $entity = $this->entity($entityName);

        return (new FacetSetResolver)->resolveEntity($entity, $context, $facetNames);
    }
    ...
    private function importFacet(FacetDefinition $source, FacetImportDefinition $import, ?string $owningModelClass, ?FacetDefinition $local = null): FacetDefinition
    {
        ...
        if ($source->getFacetGroup() !== null) {
            $facet->facetGroup($source->getFacetGroup());
        }
        ...
        if ($local !== null) {
            ...
            if ($local->getFacetGroup() !== null) {
                $facet->facetGroup($local->getFacetGroup());
            }
