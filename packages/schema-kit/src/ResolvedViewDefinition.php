...
final class ResolvedViewDefinition
{
    ...
    public function valuePath(string $name): string
    {
        if (! str_contains($name, '.')) {
            return $this->field($name)->name();
        }
        ...
        [$facetName, $fieldName] = explode('.', $name, 2);
        $facet = $this->facet($facetName);
        $resolvedFieldName = $this->field($name)->name();
        $dataPath = $facet->getDataPath();

        if (! is_string($dataPath) || $dataPath === '') {
            return $resolvedFieldName;
        }

        $dataPath = str_starts_with($dataPath, 'data.')
            ? substr($dataPath, 5)
            : $dataPath;
        ...
        return "{$dataPath}.{$resolvedFieldName}";
    }
}
