...
class QueryExpressionWalker
{
    ...
    public function apply(Builder $query, string $expression, mixed $value = null): void
    {
        ...
        $parsed = ($this->parser ??= new ExpressionParser)->parse($normalised, $names);
    ...
    private function applyNode(Builder $query, Node $node, mixed $value = null, string $boolean = 'and'): void
    {
        ...
        if ($node instanceof NameNode || $node instanceof GetAttrNode) {
            $reference = $this->resolver->resolve($node);
            $this->applyResolvedFieldCondition($query, $reference, true, $boolean);
    ...
    private function applyComparisonNode(Builder $query, BinaryNode $node, mixed $value, string $boolean): void
    {
        ...
        $reference = $this->resolver->resolve($left);
        ...
        if ($reference->isExpression()) {
            if (! is_bool($resolvedValue)) {
                throw new \InvalidArgumentException(sprintf(
                    'Expression-backed path [%s] can only be compared to true/false.',
                    $reference->path ?? 'unknown'
                ));
            }

            $this->applyResolvedFieldCondition($query, $reference, $resolvedValue, $boolean);

            return;
        }

        if ($reference->isSubquery()) {
            $this->applySubqueryCondition($query, $reference->query, $operator, $resolvedValue, $boolean);

            return;
        }

        if ($reference->isRelationship()) {
    ...
    private function applyResolvedFieldCondition(Builder $query, ResolvedExpressionReference $reference, bool $expected, string $boolean): void
    {
        if ($reference->isExpression()) {
            $method = $expected
                ? ($boolean === 'or' ? 'orWhere' : 'where')
                : ($boolean === 'or' ? 'orWhereNot' : 'whereNot');

            $query->$method(function (Builder $q) use ($reference): void {
                $this->apply($q, $reference->expression);
            });
        ...
        if ($reference->isRelationship()) {
            ...
            $query->$method($reference->relationship);

            return;
        }

        if ($reference->isSubquery()) {
            $query->where($reference->query, '=', $expected, $boolean);

            return;
        }

        $query->where($reference->column, '=', $expected, $boolean);
    }
