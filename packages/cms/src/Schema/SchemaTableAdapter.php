...
use Coda\Cms\Layouts\CardLayout;
...
class SchemaTableAdapter
{
    ...
    public function table(string $entityName, string $viewName): Table
    {
        ...
        $listLayout = $resolved->view()->getMeta('list_layout');

        if ($listLayout instanceof ListLayout) {
            $this->applyListLayout($instance, $listLayout);
        }

        return $instance;
    }

    private function applyListLayout(Table $table, ListLayout $layout): void
    {
        $cards = $layout->getCards();

        if ($cards instanceof CardLayout) {
            $definition = $cards->getDefinition();

            if ($definition !== null) {
                $table->cardDefinition($definition);
            } elseif ($cards->getFields() !== null) {
                $table->cardDefinition(CardDefinition::fromDisplayFields(
                    $cards->getFields(),
                    $cards->getTitleField(),
                    $cards->getView(),
                ));
            ...
            if ($cards->getMinWidth() !== null) {
                $table->cardMinWidth($cards->getMinWidth());
            }

            if ($cards->getItemClass() !== null) {
                $table->cardItemClass($cards->getItemClass());
            }

            if ($cards->getTitleField() !== null) {
                $table->cardTitleField($cards->getTitleField());
            }

            if ($cards->getView() !== null) {
                $table->cardView($cards->getView());
            }

            if ($cards->getMasonryOverlayView() !== null) {
                $table->masonryOverlayView($cards->getMasonryOverlayView());
            }

            if ($cards->getUrlField() !== null) {
                $table->cardUrlField($cards->getUrlField());
            }

            if ($cards->getUrlUsing() !== null) {
        ...
        if ($layout->getPagination() !== null) {
            $table->pagination($layout->getPagination());
        }

        $table->showViewToggle($layout->shouldShowViewToggle());

        if ($layout->getDefaultView() !== null) {
            $table->defaultView($layout->getDefaultView());
        }
    }
    ...
    private function identityColumn(ResolvedViewDefinition $resolved, array $config): Text
    {
        ...
        try {
            $titleFieldKey = $resolved->valuePath($titleField);
        } catch (\Throwable) {
            $titleFieldKey = $titleField;
        }
        ...
        if ($imageDefinition instanceof IdentityImageDefinition && is_string($imageDefinition->getField()) && $imageDefinition->getField() !== '') {
            ...
            try {
                $imageFieldKey = $resolved->valuePath($imageFieldKey);
            } catch (\Throwable) {
            ...
            if (is_string($imageField) && $imageField !== '') {
                ...
                try {
                    $imageFieldKey = $resolved->valuePath($imageField);
                } catch (\Throwable) {
                    $imageFieldKey = $imageField;
                }
    ...
    private function normalizeColumnFieldConfig(ResolvedViewDefinition $resolved, array $config): array
    {
        ...
        try {
            $config['field'] = $resolved->valuePath($fieldReference);
        } catch (\Throwable) {
