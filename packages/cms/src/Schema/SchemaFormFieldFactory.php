...
class SchemaFormFieldFactory
{
    public function __construct(
        private readonly EntityOptionsResolver $entityOptionsResolver,
    ) {}
    ...
    private function applyFieldMeta(Field $instance, FieldDefinition $field): void
    {
        ...
        if (is_string($meta['update_on'] ?? null) && method_exists($instance, 'updateOn')) {
            $instance->updateOn(
                $meta['update_on'],
                is_int($meta['debounce_update'] ?? null) ? $meta['debounce_update'] : null,
            );
        }
        ...
        if (is_int($meta['debounce_update'] ?? null) && method_exists($instance, 'debounceUpdate')) {
            $instance->debounceUpdate($meta['debounce_update']);
        }
    ...
    private function applyTypedInput(Field $instance, InputDefinition $input): void
    {
        ...
        if (is_string($input->getUpdateOn()) && method_exists($instance, 'updateOn')) {
            $instance->updateOn($input->getUpdateOn(), $input->getDebounceUpdate());
        } elseif (is_int($input->getDebounceUpdate()) && method_exists($instance, 'debounceUpdate')) {
            $instance->debounceUpdate($input->getDebounceUpdate());
        }
    ...
    private function applySelectInput(Field $instance, SchemaSelectInput $input): void
    {
        ...
        if ($options instanceof EntityOptions && method_exists($instance, 'optionsUsing')) {
            $instance->optionsUsing(fn (array $context = []): array => $this->entityOptionsResolver->resolveList($options, $context));
        }
    ...
    private function applyTreeInput(Field $instance, SchemaTreeInput $input): void
    {
        ...
        if ($options instanceof EntityOptions) {
            if (method_exists($instance, 'treeOptionsUsing')) {
                $instance->treeOptionsUsing(fn (array $context = []): array => $this->entityOptionsResolver->resolveTree($options, $context));
            } elseif (method_exists($instance, 'optionsUsing')) {
                $instance->optionsUsing(fn (array $context = []): array => $this->entityOptionsResolver->resolveTree($options, $context));
            }
        }
