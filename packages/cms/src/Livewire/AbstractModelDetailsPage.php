...
abstract class AbstractModelDetailsPage extends Component
{
    ...
    public function save(): void
    {
        ...
        if ($hasMedia) {
            $mediaModel = $this->resolveMediaModelForDataClass($dataClass, (int) ($instance->id ?? $this->record), $model);
    ...
    public function initializeTabState(string $default): void
    {
        ...
        $this->initializeAllowedTabs($validTabs, $default);
    }

    /**
     * @param  array<int, string>  $validTabs
     */
    protected function initializeAllowedTabs(array $validTabs, ?string $default = null): void
    {
        $validTabs = array_values(array_filter(
            $validTabs,
        ...
        if ($default !== '' && ($this->activeTab === '' || ($validTabs !== [] && ! in_array($this->activeTab, $validTabs, true)))) {
    ...
    protected function loadRecord(): void
    {
        ...
        unset($this->fieldsets, $this->detailsLayout, $this->detailsFieldsetsByName);
    ...
    protected function loadExistingMediaFromModel(Model $model): void
    {
        ...
        $dataClass = $this->dataClass();
        $mediaModel = $this->resolveMediaModelForDataClass($dataClass, $this->record, $model);

        if ($mediaModel instanceof HasMedia) {
            $this->loadAllExistingMedia($mediaModel);
        }
    }

    protected function resolveMediaModelForDataClass(string $dataClass, int $id, ?Model $fallback = null): ?Model
    {
        if (is_subclass_of($dataClass, ResolvesMediaModel::class)) {
            return $dataClass::resolveMediaModel($id);
        }
