...
{
    use Concerns\InteractsWithScopeContext;
    ...
    {
        return $this->resolveScopedContextBindings($this->getModalContextBindings($formDataClass));
    }

    protected function getModalExcludedFields(string $formDataClass): array
    {
        return array_keys($this->getModalContextBindings($formDataClass));
    }

    protected function getModalContextBindings(string $formDataClass): array
    {
        return [];
    }
    ...
    protected function resolveScopedContextBindings(array $bindings): array
    {
    ...
    protected function resolveFormModalMaxWidth(?string $formDataClass, string $default = 'max-w-sm'): string
    {
        ...
        return is_string($width) && $width !== '' ? $width : $default;
    }
    ...
    public function formModals(): array
    {
        ...
        foreach ($this->actions as $action) {
            ...
            $modals[] = [
                ...
                'maxWidth' => $this->resolveFormModalMaxWidth($action->formDataClass),
            ];
        }
