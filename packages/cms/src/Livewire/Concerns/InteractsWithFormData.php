...
trait InteractsWithFormData
{
    ...
    public function updated(string $property, mixed $value): void
    {
        ...
        if (str_starts_with($property, 'data.')) {
            ...
            if ($this->getErrorBag()->has($property)) {
                $this->validateFormField($property);
            }
        }
    }
    ...
    public function validateFormField(string $property): void
    {
        $rules = array_merge(
            $this->buildValidationRulesFromFieldsets(),
            method_exists($this, 'buildMediaValidationRules') ? $this->buildMediaValidationRules() : []
        );
    ...
    public function applyRelationshipSelectorClientState(string $fieldName, array $items = [], array $modalState = []): void
    {
        $field = $this->findField($fieldName);
    ...
    {
        if (is_string($field->clientModalInitialListKey ?? null) && $field->clientModalInitialListKey !== '') {
            return $field->clientModalInitialListKey;
        }
    ...
    private function readWireModel(string $wireModel): mixed
    {
        if (str_starts_with($wireModel, 'data.')) {
            return data_get($this->data, substr($wireModel, 5));
        }

        return data_get($this, $wireModel);
    }

    private function writeWireModel(string $wireModel, mixed $value): void
    {
        if (str_starts_with($wireModel, 'data.')) {
            data_set($this->data, substr($wireModel, 5), $value);

            return;
        }

        data_set($this, $wireModel, $value);
    }
}
