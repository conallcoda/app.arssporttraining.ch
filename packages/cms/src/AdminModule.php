...
use Coda\Cms\Layouts\ListLayout;
...
use Coda\Cms\Schema\SchemaFormFieldFactory;
...
class AdminModule extends Module
{
    ...
    private ?string $moduleListComponent = null;

    private ?Closure $queryUsing = null;

    private string|Closure|null $pageBreadcrumbLabel = null;

    private ?string $pageParent = null;

    private string|Closure|null $detailsBreadcrumbLabel = null;

    private ?string $detailsParent = null;
    ...
    public function hideFromNavigation(): static
    {
    ...
    public function listComponent(string $component): static
    {
        $this->moduleListComponent = $component;
    ...
    public function pageBreadcrumbLabel(string|Closure|null $label): static
    {
        $this->pageBreadcrumbLabel = $label;

        return $this;
    }

    public function pageParent(?string $parent): static
    {
    ...
    public function detailsBreadcrumbLabel(string|Closure|null $label): static
    {
        $this->detailsBreadcrumbLabel = $label;

        return $this;
    }

    public function detailsParent(?string $parent): static
    {
        $this->detailsParent = $parent;
    ...
    public function table(TableLayout $layout): static
    {
        ...
        return $this->list(ListLayout::make()->table($layout));
    }

    public function list(ListLayout $layout): static
    {
        $this->listLayout = $layout;

        $tableLayout = $layout->getTable() ?? TableLayout::make();
            ...
            View::make('cms_list')
                ...
                ->table(
                    TableViewDefinition::make()
                        ->columns($tableLayout->getColumns())
                        ->sortable($tableLayout->getSortable())
                        ->filters($tableLayout->getFilters())
                        ->defaultSort(
                            $tableLayout->getDefaultSort()['field'] ?? 'id',
                            $tableLayout->getDefaultSort()['direction'] ?? 'asc',
                        )
                )
                ->setMeta('list_layout', $layout)
        );
    ...
    public function pages(): array
    {
        return [
            PageDefinition::make($this->moduleName)
                ...
                ->breadcrumbLabel($this->pageBreadcrumbLabel)
                ->component($this->moduleListComponent ?? AdminModelListPage::class)
    ...
    {
        return $this->showInNavigation ? parent::navigationRoute() : null;
    }
    ...
    public function detailPages(): array
    {
        ...
        $page = $this->detailsPage(
            ...
            listComponent: $this->moduleListComponent ?? AdminModelListPage::class,
            component: $this->moduleDetailsComponent ?? AdminModelDetailsPage::class,
        )
            ->parent($this->detailsParent)
            ->breadcrumbLabel($this->detailsBreadcrumbLabel)
            ->middleware($this->moduleMiddleware);
    ...
    {
        $query = $this->baseQuery($component);

        $with = $this->requireEntityDefinition()->getWith();
    ...
    private function schemaPresenter(): CmsSchemaPresenter
    {
        ...
        return CmsSchemaPresenter::make(
            $registry,
            app(SchemaFormFieldFactory::class),
        );
    }
    ...
    private function scopeContextBindings(): array
    {
        ...
        foreach ($this->activeScopes as $scopeKey) {
            ...
            $bindings[$field] = fn (mixed $context = null, mixed $component = null) => $this->currentScopeValueFor($scope, $component, $context);
        }

        return $bindings;
    }

    private function baseQuery(object $component): Builder
    {
        if ($this->queryUsing instanceof Closure) {
            ...
            throw new \InvalidArgumentException("Admin module [{$this->moduleName}] queryUsing closure must return an Eloquent builder.");
        }

        $modelClass = $this->moduleRecordModel ?? $this->requireEntityDefinition()->getModelClass();

        if (! is_string($modelClass) || $modelClass === '') {
            throw new \InvalidArgumentException("Admin module [{$this->moduleName}] is missing a queryUsing closure or record model.");
        }
    ...
    {
        $scope = $this->requireEntityDefinition()->requireScope($scopeKey);
        $value = $this->currentScopeValueFor($scope, $component);
    ...
    private function currentScopeValueFor(ScopeDefinition $scope, mixed $component = null, mixed $context = null): mixed
    {
        if ($context !== null) {
            if ($context instanceof \Illuminate\Database\Eloquent\Model && ($scope->getContextPath() === 'id' || $scope->getContextPath() === $context->getKeyName())) {
                return $context->getKey();
            }

            return data_get($context, $scope->getContextPath());
        }

        if ($component !== null && method_exists($component, 'scopeValue')) {
            return $component->scopeValue($scope->getContextPath());
        }
