@php
    ...
    $columnCount = $manualSortingEnabled ? 3 : 2;
@endphp
    ...
    @foreach ($items as $item)
        ...
        <flux:table.row
            ...
            @if ($manualSortingEnabled)
                <flux:table.cell class="{{ $parentCellClass }}">
                    @if ($isManualSortItem)
                        <button
                            type="button"
                            x-sort:handle
            ...
            <flux:table.cell class="{{ $parentCellClass }}">
                <div class="flex items-center gap-1" style="padding-left: {{ $item->depth * 1.5 }}rem">
                    ...
                    @else
                        <span class="w-5"></span>
                    @endif
                    <span class="{{ $parentLabelClass }}">{{ $item->name }}</span>
                </div>
            </flux:table.cell>
                ...
                <flux:table.cell colspan="{{ $columnCount }}" class="pt-0 pb-4">
                    ...
                    >
                        @foreach ($item->children as $child)
                            ...
                            @endphp
                            <div x-sort:item="{{ $childSortKey }}" data-sort-item-key="{{ $childSortKey }}" class="{{ $cardItemClass }}">
