@php
    ...
    $fieldValue = data_get($item, $column->field);
    $formattedTextValue = $column instanceof TextColumn ? $column->formatValue($fieldValue) : null;
@endphp
...
@if ($column instanceof RelationshipColumn)
    @php $relation = $fieldValue; @endphp
    <div class="py-1 flex flex-wrap gap-1">
            ...
            @foreach ($relation as $index => $related)
                @if ($column->modalField)
                    <flux:badge size="sm" class="cursor-pointer"
                        wire:click.stop="startEdit({{ $item->id }}, {{ Js::from($column->modalField) }}, {{ $index }})">
        ...
        @elseif ($relation)
            @if ($column->modalField)
                <flux:badge size="sm" class="cursor-pointer"
                    wire:click.stop="startEdit({{ $item->id }}, {{ Js::from($column->modalField) }}, 0)">
    ...
    <div class="py-1 flex flex-wrap gap-1">
        @if ($column->source)
            ...
            @foreach ($sourceBadges as $badge)
                ...
                @if (isset($badge['url']))
                    <a href="{{ $badge['url'] }}">
                        <flux:badge size="sm" class="cursor-pointer hover:opacity-80 {{ $badgeColorClass }}">
                            {{ $badge['label'] }}</flux:badge>
                    </a>
                @elseif (isset($badge['modalField']))
                    <flux:badge size="sm" class="cursor-pointer {{ $badgeColorClass }}"
                        wire:click.stop="startEdit({{ $item->id }}, {{ Js::from($badge['modalField']) }})">
                        {{ $badge['label'] }}</flux:badge>
                @elseif ($badgeColorClass)
        ...
        @else
            ...
            @foreach ($badgeValues as $index => $val)
                @if ($val !== null && $val !== '')
                    @if ($column->modalField)
                        <flux:badge size="sm" class="cursor-pointer"
                            wire:click.stop="startEdit({{ $item->id }}, {{ Js::from($column->modalField) }})">
...
@elseif ($column instanceof ColorBadgeColumn)
    @php $colorValue = $fieldValue; @endphp
    @if ($colorValue)
...
@elseif ($column instanceof TextWithBadgeGroupsColumn)
    <div class="py-1 flex flex-wrap items-center gap-1">
        ...
        @else
            <span class="mr-1">{{ $fieldValue }}</span>
        @endif
...
@elseif ($column instanceof ViewColumn)
    <div class="{{ $baseTextClasses }}">
        <a href="{{ route($column->getViewRouteName(), $model) }}"
            ...
            {{ $column->formatValue($fieldValue) }}{{ $column->suffix }}
        </a>
    </div>
...
@elseif ($column instanceof TextColumn && $column->imageField && property_exists($column, 'modalField') && $column->modalField)
    <div class="{{ $baseTextClasses }} cursor-pointer hover:bg-zinc-100 dark:hover:bg-zinc-800 rounded flex items-center gap-3 min-w-0"
        wire:click.stop="startEdit({{ $item->id }})">
        {!! $renderTextImage() !!}
