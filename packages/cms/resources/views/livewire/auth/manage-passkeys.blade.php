@php
    $realmKey = config('cms.auth.realm');
    $registerRoutes = [];
    $passkeyUserId = auth()->id();

    if ($realmKey && class_exists(\Coda\AuthKit\AuthRegistry::class)) {
        $registerRoutes = [
            'register' => [
                'options' => route("auth-kit.{$realmKey}.passkey.registration-options"),
                'submit' => route("auth-kit.{$realmKey}.passkey.store"),
            ],
        ];
    }
@endphp
...
<div
    x-data="cmsPasskeys({ csrfToken: @js(csrf_token()), routes: @js($registerRoutes), realm: @js($realmKey), userId: @js($passkeyUserId) })"
    x-on:passkey-registered.window="$wire.dispatch('passkey-registered')"
    x-on:passkey-deleted.window="if (window.authKitForgetPasskeyOnDevice) { window.authKitForgetPasskeyOnDevice(@js($realmKey), @js($passkeyUserId)) }"
    class="space-y-6"
>
    <div>
