...
use Coda\Cms\Tests\Fixtures\TestConfiguredCardList;
use Coda\Cms\Tests\Fixtures\TestInfiniteList;
use Coda\Cms\Tests\Fixtures\TestItemList;
use Coda\Cms\Tests\Fixtures\TestManualSortItemList;
use Coda\Cms\Tests\Fixtures\TestNestedFieldItemList;
...
beforeEach(function () {
    ...
    Livewire::component('test-configured-card-list', TestConfiguredCardList::class);
    Livewire::component('test-manual-sort-item-list', TestManualSortItemList::class);
    Livewire::component('test-nested-field-item-list', TestNestedFieldItemList::class);
});
...
it('renders nested text field paths', function () {
    CmsTestItem::factory()->create(['name' => 'Alpha', 'status' => 'Speaker']);

    Livewire::test(TestNestedFieldItemList::class)
        ->assertOk()
        ->assertSee('Alpha')
        ->assertSee('Speaker');
});
...
it('uses the sort_order column when manual sorting is enabled', function () {
    CmsTestItem::factory()->create(['name' => 'Alpha', 'sort_order' => 1]);
    CmsTestItem::factory()->create(['name' => 'Zulu', 'sort_order' => 0]);

    $items = Livewire::test(TestManualSortItemList::class)->instance()->items();
    $names = $items->pluck('name')->all();

    expect($names)->toBe(['Zulu', 'Alpha']);
});

it('ignores clickable column sorting when manual sorting is enabled', function () {
    $instance = Livewire::test(TestManualSortItemList::class)->instance();
    $instance->sortBy('name');

    expect($instance->sort)->toBe('');
});

it('reorders the current page through manual sorting', function () {
    $a = CmsTestItem::factory()->create(['name' => 'A', 'sort_order' => 0]);
    $b = CmsTestItem::factory()->create(['name' => 'B', 'sort_order' => 1]);

    Livewire::test(TestManualSortItemList::class)
        ->call('reorderCurrentPage', [$b->id, $a->id]);
...
it('exposes cardDefinition when configured explicitly', function () {
    $instance = Livewire::test(TestConfiguredCardList::class)->instance();

    expect($instance->cardDefinition)->not->toBeNull();
    expect($instance->cardDefinition->titleField?->field)->toBe('name');
    expect($instance->cardDefinition->subtitleField?->field)->toBe('status');
});
