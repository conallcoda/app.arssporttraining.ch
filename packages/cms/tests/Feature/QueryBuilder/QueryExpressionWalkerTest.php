...
use Coda\Cms\QueryBuilder\WhitelistFieldResolver;
...
beforeEach(function () {
    ...
    $this->walker = new QueryExpressionWalker(new WhitelistFieldResolver(whitelist()));
});
...
{
    $query = articleQuery();
    $wl = $fieldWhitelist ?? whitelist();
    $resolver = new WhitelistFieldResolver($wl, str_contains($expression, '$value'));
...
it('filters rows with equals', function () {
    ...
    $query = articleQuery();
    $this->walker->apply($query, 'status == "published"');
...
it('filters rows with and', function () {
    ...
    $query = articleQuery();
    $this->walker->apply($query, 'status == "published" and year >= 2024');
...
it('filters rows with or', function () {
    ...
    $query = articleQuery();
    $this->walker->apply($query, 'status == "published" or status == "archived"');
...
it('filters rows with contains and value placeholder', function () {
    ...
    $query = articleQuery();
    $this->walker->apply($query, 'title contains $value', 'Strength');
...
it('filters rows with in operator', function () {
    ...
    $query = articleQuery();
    $this->walker->apply($query, 'status in ["draft", "archived"]');
