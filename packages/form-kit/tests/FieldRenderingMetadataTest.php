...
it('uses semantic update policies to build wire:model directives', function () {
    ...
    expect($field->wireModelDirective())->toBe('wire:model.live.blur');

    $field->updateOn('live', 250);
    expect($field->wireModelDirective())->toBe('wire:model.live.debounce.250ms');

    $field->change();
    expect($field->wireModelDirective())->toBe('wire:model.live.change');
...
it('resolves uncached dynamic options for the current context', function () {
    $field = Select::make('flow')
        ->optionsUsing(fn (array $context) => [
            $context['area_id'] => 'Area '.$context['area_id'],
        ]);

    expect($field->getOptions(['area_id' => 1]))->toBe([1 => 'Area 1'])
        ->and($field->getOptions(['area_id' => 2]))->toBe([2 => 'Area 2']);
});
