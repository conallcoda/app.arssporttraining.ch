...
@php
    ...
    $activeTabKey = $wireModel ? md5((string) data_get($this, $wireModel, '')) : '';
...
@if ($showWrapper)
    <fieldset class="min-w-0 border border-zinc-200 dark:border-zinc-700 rounded-lg p-4 space-y-4 [&>legend+*]:!mt-0 {{ $group->class }}">
        ...
        @endforeach
        @if ($wireModel)
            <flux:tab.group wire:key="tab-group-{{ $tabKey }}-{{ $activeTabKey }}">
                ...
                @foreach ($group->fieldsets as $item)
                    <flux:tab.panel :name="$item->name" class="pt-6">
                        ...
                        @else
                            <x-form-kit::form.fieldset :fieldset="$item" :prefix="$item->prefix ?? 'data'" :showLegend="false" :context-data="$item->contextData" />
                        @endif
                    </flux:tab.panel>
                @endforeach
            </flux:tab.group>
