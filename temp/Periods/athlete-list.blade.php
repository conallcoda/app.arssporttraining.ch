        ...
        <flux:table.rows>
            @foreach ($this->athletes as $user)
                ...
                <flux:table.row wire:key="athlete-{{ $ath->id }}">
                    <flux:table.cell>
                        <div x-data="editable_cell($wire, 'update', [{{ $ath->id }}, 'forename'], {{ json_encode($ath->forename) }}, '', false)" @click="startEditing" class="cursor-pointer w-full">
                            <div x-show="!editing" x-text="value" class="px-2 py-1 truncate border border-transparent">
                            </div>
                            <input x-show="editing" x-cloak x-ref="input" x-model="value" @click.stop @blur="save"
                                @keydown="handleKeydown" type="text"
                    ...
                    <flux:table.cell>
                        <div x-data="editable_cell($wire, 'update', [{{ $ath->id }}, 'surname'], {{ json_encode($ath->surname) }}, '', false)" @click="startEditing" class="cursor-pointer w-full">
                            <div x-show="!editing" x-text="value" class="px-2 py-1 truncate border border-transparent">
                            </div>
                            <input x-show="editing" x-cloak x-ref="input" x-model="value" @click.stop @blur="save"
                                @keydown="handleKeydown" type="text"
                    ...
                    @foreach ($ath->tests as $testIndex => $test)
                        <flux:table.cell>
                            <div x-data="editable_cell($wire, 'update', [{{ $ath->id }}, 'test.{{ $testIndex }}.reps'], {{ $test->reps }})" @click="startEditing" class="cursor-pointer w-full">
                                ...
                                <input x-show="editing" x-cloak x-ref="input" x-model="value" @click.stop
                                    @blur="save" @keydown="handleKeydown" type="number" step="1" min="1"
                                    class="w-full px-2 py-1 text-sm border border-zinc-300 dark:border-zinc-600 rounded outline-none focus:border-zinc-500 focus:ring-0" />
                            </div>
                        </flux:table.cell>

                        <flux:table.cell>
                            <div x-data="editable_cell($wire, 'update', [{{ $ath->id }}, 'test.{{ $testIndex }}.weight'], {{ $test->weight }}, ' kg')" @click="startEditing" class="cursor-pointer w-full">
                                <div x-show="!editing" x-text="value + ' kg'"
                                    class="px-2 py-1 border border-transparent"></div>
                                <input x-show="editing" x-cloak x-ref="input" x-model="value" @click.stop
                                    @blur="save" @keydown="handleKeydown" type="number" step="0.5" min="1"
                                    class="w-full px-2 py-1 text-sm border border-zinc-300 dark:border-zinc-600 rounded outline-none focus:border-zinc-500 focus:ring-0" />
                            </div>
                        </flux:table.cell>

                        <flux:table.cell>
                            <div class="px-2 py-1 border border-transparent">{{ number_format($test->oneRepMax, 1) }}
                                kg</div>
                        </flux:table.cell>
                    @endforeach

                    <flux:table.cell>
                        <div x-data="editable_cell($wire, 'update', [{{ $ath->id }}, 'target_modifier'], {{ $ath->target_modifier }}, '%')" @click="startEditing" class="cursor-pointer w-full">
                            <div x-show="!editing" x-text="value + '%'" class="px-2 py-1 border border-transparent">
                            </div>
                            <input x-show="editing" x-cloak x-ref="input" x-model="value" @click.stop @blur="save"
                                @keydown="handleKeydown" type="number" step="0.1" min="0"
                                class="w-full px-2 py-1 text-sm border border-zinc-300 dark:border-zinc-600 rounded outline-none focus:border-zinc-500 focus:ring-0" />
                        </div>
                    </flux:table.cell>
