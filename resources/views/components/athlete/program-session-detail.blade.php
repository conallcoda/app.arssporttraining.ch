...
<div>
    <div @class([
        ...
        <div class="w-full md:mx-auto md:max-w-[900px] border-b border-zinc-200 dark:border-zinc-700">
            <div class="athlete-toolbar__row w-full">
                @if ($backMode === 'action')
                    <button
                        ...
                        class="inline-flex min-h-11 w-full items-center gap-3 bg-zinc-800/5 px-4 py-3 text-left text-sm text-zinc-700 transition hover:bg-white dark:bg-white/10 dark:text-white dark:hover:bg-white/20"
                        aria-label="{{ $backLabel }}"
                    >
                        <flux:icon.chevron-left class="size-4 shrink-0" />
                        <span class="truncate font-medium">{{ $backLabel }}</span>
                    </button>
                @else
                    <a
                        href="{{ $backHref }}"
                        wire:navigate
                        @if ($backUseHistory) onclick="if (window.history.length > 1) { event.preventDefault(); window.history.back(); }" @endif
            ...
            <div class="flex w-full items-center gap-3 border-t border-zinc-200 bg-zinc-50 px-3 py-2.5 dark:border-zinc-700 dark:bg-zinc-900/40">
                <x-athlete.category-badge class="shrink-0" :label="$categoryLabel" :color="$categoryColor" />
            ...
            @if ($showsSectionTabs)
                <div class="border-t border-zinc-200 bg-zinc-50 px-0 py-0 dark:border-zinc-700 dark:bg-zinc-900/40">
                    <x-athlete.segmented-tabs :tabs="$sectionTabItems" model="{{ $sectionModel }}" />
                </div>
            @endif
        </div>
    </div>
            ...
            @forelse ($programExercises as $exercise)
                <x-athlete.section :row="$loop->iteration" class="py-0" content-class="space-y-0 py-3" wire:key="exercise-{{ $exercise->id }}" x-data="{ open: {{ $autoExpandExerciseId === $exercise->id ? 'true' : 'false' }} }" x-on:athlete-exercise-action-succeeded.window="if (($event.detail.exerciseId ?? null) === {{ $exercise->id }}) open = false">
                    <div x-on:click="open = !open" x-on:keydown.enter.prevent="open = !open"
                        ...
                        <flux:icon.chevron-up x-show="open" x-cloak class="mt-1 size-5 shrink-0 text-zinc-500 dark:text-zinc-400" />
                        <flux:icon.chevron-down x-show="!open" x-cloak class="mt-1 size-5 shrink-0 text-zinc-500 dark:text-zinc-400" />
                    </div>
                    ...
                    <div x-show="open" x-cloak x-collapse x-bind:class="open ? 'pt-3' : 'pt-0'" class="space-y-3">
                        <div class="space-y-2">
                            @if (!empty($exercise->equipmentBadges) || !empty($exercise->modifierBadges))
                                <div class="flex flex-wrap gap-2">
                                    @foreach ($exercise->equipmentBadges as $badge)
                                        <flux:badge color="blue" size="sm" class="text-[10px]">{{ $badge }}</flux:badge>
                                    @endforeach

                                    @foreach ($exercise->modifierBadges as $badge)
                                        <flux:badge size="sm" class="text-[10px]">{{ $badge }}</flux:badge>
                                    @endforeach
                                </div>
                            @endif
                            ...
                            @if ($exercise->instructions)
                                <flux:text class="whitespace-pre-line text-[11px] text-zinc-600 dark:text-zinc-300">
                                    ...
                                    <table class="min-w-full border-collapse text-xs">
                                        <thead>
                                            <tr class="bg-zinc-100 dark:bg-zinc-800">
                                                <th class="sticky left-0 z-10 border-b border-r border-zinc-300 bg-zinc-100 px-2 py-1.5 text-center font-semibold dark:border-zinc-600 dark:bg-zinc-800 whitespace-nowrap">
                                                    {{ $exercise->setLabel }}
                                                </th>
                                                @foreach ($exercise->sessionRows as $row)
                                                    <th class="border-b border-r border-zinc-300 px-2 py-1.5 text-center font-semibold last:border-r-0 dark:border-zinc-600 whitespace-nowrap {{ $row->labelClass }}">
                                                        {{ $row->label }}
                                                    </th>
                                                @endforeach
                                            </tr>
                                        ...
                                        <tbody>
                                            @for ($set = 0; $set < $exercise->setCount; $set++)
                                                <tr>
                                                    <td class="sticky left-0 z-10 border-r border-t border-zinc-300 bg-zinc-100 px-2 py-1.5 text-center font-medium dark:border-zinc-600 dark:bg-zinc-800 whitespace-nowrap">
                                                        {{ $set + 1 }}
                                                    </td>
                                                    @foreach ($exercise->sessionRows as $row)
                                                        @php
