<div>
    @unless ($compact)
        <div class="flex items-center justify-between mb-3">
            <flux:tabs wire:model.live="typeFilter" variant="segmented" size="sm">
                <flux:tab name="all">All <flux:badge size="sm" inset="right">{{ $this->typeCounts['all'] ?? 0 }}</flux:badge></flux:tab>
                @foreach ($exerciseTypes as $exerciseType)
                    <flux:tab :name="$exerciseType->value">{{ $exerciseType->label() }} <flux:badge size="sm" inset="right">{{ $this->typeCounts[$exerciseType->value] ?? 0 }}</flux:badge></flux:tab>
                @endforeach
            </flux:tabs>
                ...
                @foreach ($this->headerActions as $action)
                    <flux:button variant="{{ $action->variant ?? 'primary' }}" size="sm" icon="{{ $action->icon }}"
                        x-on:click="Livewire.dispatch('open-{{ $this->getModalNameForAction($action) }}', {
                            ...
                            data: $wire.typeFilter !== 'all' ? { type: $wire.typeFilter } : {}
                        })">
                        {{ $action->label }}
                    </flux:button>
                @endforeach
