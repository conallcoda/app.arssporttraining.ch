...
<div class="overflow-x-auto" x-data="calendar_slot_popover({ groupId: '{{ $this->group }}', userId: '{{ $this->user }}', startDate: '{{ $gridStart->format('Y-m-d') }}', endDate: '{{ $gridEnd->format('Y-m-d') }}', gridCellsUrl: '{{ route('api.program-grid-cells') }}', slotDetailsUrl: '{{ route('api.slot-details') }}' })">
        ...
        @if ($this->hasSelection())
            <tbody x-data="{ expanded: $persist(false).as('cal-metrics') }"
                ...
                <tr>
                    ...
                    @foreach ($this->days as $day)
                        <td class="border-r border-b border-zinc-300 dark:border-zinc-600 p-0.5 {{ $day['oddWeek'] ? 'bg-zinc-50/50 dark:bg-zinc-700/10' : '' }}">
                            ...
                            @foreach ($this->days as $day)
                                ...
                                @if ($this->user !== '')
                                    <td wire:click="openMetricCell('{{ $metricCase->value }}', '{{ $day['date'] }}')"
                                        ...
                                        class="group/cell border-r border-b border-zinc-300 dark:border-zinc-600 p-0.5 cursor-pointer hover:brightness-95 dark:hover:brightness-125 {{ $day['oddWeek'] ? 'bg-zinc-50/50 dark:bg-zinc-700/10' : '' }}">
                                        @if ($metricCell)
                                ...
                                @elseif ($this->group !== '')
                                    ...
                                    @endphp
                                    @if ($groupCell)
                                        <td class="border-r border-b border-zinc-300 dark:border-zinc-600 p-0.5 hover:brightness-95 dark:hover:brightness-125 {{ $day['oddWeek'] ? 'bg-zinc-50/50 dark:bg-zinc-700/10' : '' }}">
                    ...
                    @foreach ($this->days as $dayIdx => $day)
                        ...
                        @if ($inBlock)
                            <td wire:click="editBlock({{ $blockId }})"
                                title="{{ $blockNote }}"
                                class="border-r border-b border-zinc-300 dark:border-zinc-600 p-0.5 cursor-pointer hover:brightness-95 dark:hover:brightness-125 {{ $bgClass }}"
                                :class="(endIdx !== null ? ({{ $dayIdx }} >= Math.min(anchorIdx, endIdx) && {{ $dayIdx }} <= Math.max(anchorIdx, endIdx)) : anchorIdx === {{ $dayIdx }}) && 'ring ring-inset ring-black dark:ring-white'">
                        ...
                        @else
                            <td @mousedown.stop="startDrag({{ $dayIdx }}, '{{ $day['date'] }}', $event)"
                                @mouseover="dragOver({{ $dayIdx }}, '{{ $day['date'] }}')"
                                @contextmenu="showContextMenu($event, {{ $dayIdx }}, '{{ $day['date'] }}')"
                                class="border-r border-b border-zinc-300 dark:border-zinc-600 p-0.5 cursor-pointer select-none hover:brightness-95 dark:hover:brightness-125 {{ $bgClass }}"
                                :class="(endIdx !== null ? ({{ $dayIdx }} >= Math.min(anchorIdx, endIdx) && {{ $dayIdx }} <= Math.max(anchorIdx, endIdx)) : anchorIdx === {{ $dayIdx }}) && 'ring ring-inset ring-black dark:ring-white'">
                                <div class="aspect-square" :class="hasCategoryData({{ json_encode($categoryProgramIds) }}, '{{ $day['date'] }}') && 'rounded-sm {{ $categoryColorClass }}'"></div>
                            </td>
                        @endif
                        ...
                        @foreach ($this->days as $dayIdx => $day)
                            ...
                            @if ($user !== '')
                                ...
                                @if ($slotCount > 0)
                                    ...
                                    <td wire:click="editWeekSlot({{ $entry->id }}, '{{ $day['date'] }}', '{{ $firstTime }}')"
                                        ...
                                        class="border-r border-b border-zinc-300 dark:border-zinc-600 p-0.5 cursor-pointer hover:brightness-95 dark:hover:brightness-125 {{ $programBgClass }}"
                                        :class="(endIdx !== null ? ({{ $dayIdx }} >= Math.min(anchorIdx, endIdx) && {{ $dayIdx }} <= Math.max(anchorIdx, endIdx)) : anchorIdx === {{ $dayIdx }}) && 'ring ring-inset ring-black dark:ring-white'">
                            ...
                            @else
                                <td @if ($programInBlock) title="{{ $programBlockNote }}" @endif
                                    class="border-r border-b border-zinc-300 dark:border-zinc-600 p-0.5 hover:brightness-95 dark:hover:brightness-125 {{ $programBgClass }}"
                                    :class="(endIdx !== null ? ({{ $dayIdx }} >= Math.min(anchorIdx, endIdx) && {{ $dayIdx }} <= Math.max(anchorIdx, endIdx)) : anchorIdx === {{ $dayIdx }}) && 'ring ring-inset ring-black dark:ring-white'">
