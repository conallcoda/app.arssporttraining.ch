<div class="overflow-x-auto">
    <table class="border-separate border-spacing-0 text-sm border-t border-l border-zinc-300 dark:border-zinc-600">
        ...
        <tbody>
            @foreach ($this->athleteGridData as $memberRow)
                <tr wire:key="athlete-{{ $memberRow['user']->id }}">
                    <td class="sticky left-0 z-10 border-r border-b border-zinc-300 dark:border-zinc-600 px-3 py-2 font-medium text-zinc-600 dark:text-zinc-400 whitespace-nowrap min-w-[180px] bg-white dark:bg-zinc-900">
                        <button
                            type="button"
                            wire:click="selectFromOverview({{ (int) $this->group }}, {{ $memberRow['user']->id }})"
                        ...
                        @if (! empty($memberRow['dates'][$day['date']]))
                            @php
                                ...
                                $gradient = count($stops) === 1
                                    ? "background-color: " . (array_key_first($colorCounts) === '_none' ? 'var(--color-zinc-300)' : 'var(--color-' . array_key_first($colorCounts) . '-600)') . ';'
