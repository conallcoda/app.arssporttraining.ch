<div>
    <flux:tab.group>
        <div class="sticky top-0 z-10 bg-white px-6 pt-6 dark:bg-zinc-900">
            <flux:tabs>
        ...
        <flux:tab.panel name="calculator" class="px-6">
            <div class="pt-6">
                <div class="mb-6">
                    <h1 class="text-2xl font-bold">Training Plan Calculator</h1>
                </div>
                            ...
                            <table class="w-full border-collapse border border-zinc-300 dark:border-zinc-600">
                                ...
                                <tbody>
                                    @foreach ($exercises as $ex)
                                        <tr>
                                            <td class="border border-zinc-300 px-3 py-2 dark:border-zinc-600">{{ $ex->id }}
                                            </td>
                                            <td class="border border-zinc-300 px-3 py-2 dark:border-zinc-600">{{ $ex->name }}
                                            </td>
                                            <td class="border border-zinc-300 dark:border-zinc-600 w-20 h-10 p-0"
                                                ...
                                                <div x-show="!editing" class="px-3 py-2 cursor-pointer text-center"
                                                    x-text="value + '%'"></div>
                                                <input x-show="editing" x-cloak x-ref="input" x-model="value" @blur="save"
                                                    @keydown="handleKeydown" type="number" step="0.1" min="1"
                            ...
                            <table class="w-full border-collapse border border-zinc-300 dark:border-zinc-600">
                                ...
                                <tbody>
                                    @foreach ($athletes as $ath)
                                        <tr>
                                            <td class="border border-zinc-300 px-3 py-2 dark:border-zinc-600">{{ $ath->id }}
                                            </td>
                                            <td class="border border-zinc-300 px-3 py-2 dark:border-zinc-600">{{ $ath->name }}
                                            </td>
                                            @foreach ($ath->tests as $testIndex => $test)
                                                <td class="border border-zinc-300 dark:border-zinc-600 w-20 h-10 p-0"
                                                    ...
                                                    <div x-show="!editing" class="px-3 py-2 cursor-pointer text-center"
                                                        x-text="value"></div>
                                                    <input x-show="editing" x-cloak x-ref="input" x-model="value" @blur="save"
                                                        @keydown="handleKeydown" type="number" step="1" min="1"
                                                        class="w-full h-full text-center border border-black outline-none focus:border-black focus:ring-0" />
                                                </td>
                                                <td class="border border-zinc-300 dark:border-zinc-600 w-24 h-10 p-0"
                                                    ...
                                                    <div x-show="!editing" class="px-3 py-2 cursor-pointer text-center"
                                                        x-text="value + ' kg'"></div>
                                                    <input x-show="editing" x-cloak x-ref="input" x-model="value" @blur="save"
                                                        @keydown="handleKeydown" type="number" step="0.5" min="1"
                                                        class="w-full h-full text-center border border-black outline-none focus:border-black focus:ring-0" />
                                                </td>
                    ...
                    <div class="p-4">
                        <div class="flex flex-wrap gap-4">
                            @foreach ($this->manager->results as $index => $result)
                                <x-exercise-block-grid :block="$result->current" :title="'Step ' . ($index + 1) . ': ' . $result->title()" :helpText="$result->helpText()" :highlightedCells="$result->getHighlightedCells()" />
                            @endforeach
                        </div>
                    </div>
                </div>
        ...
        <flux:tab.panel name="documentation" class="px-6">
            <div class="flex gap-8 pt-6">
                {{-- Sticky Sidebar TOC --}}
                @if (count($this->documentationToc) > 0)
                    <aside class="hidden w-64 shrink-0 lg:block">
                        <nav class="sticky top-20 max-h-[calc(100vh-6rem)] overflow-y-auto rounded-lg border border-zinc-200 bg-zinc-50 p-4 dark:border-zinc-700 dark:bg-zinc-800/50">
