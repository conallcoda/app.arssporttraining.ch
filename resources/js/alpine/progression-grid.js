    ...
    Alpine.data('progression_grid', () => ({
        ...
        setCellValue(rowIndex, colIndex, value, dispatchEvent = true) {
            ...
            if (el) {
                ...
                if (dispatchEvent) {
                    this.dispatchOverride(rowIndex, colIndex, value);
                }
            }
        },

        dispatchOverride(rowIndex, colIndex, value) {
            const key = `${rowIndex}-${colIndex}`;
            ...
            if (meta.field === 'reps') {
                params.reps = numericValue !== null ? Math.round(numericValue) : null;
            } else if (meta.field === 'weight') {
                params.weight = numericValue;
            }

            if (numericValue === null) {
                this.$dispatch('progression-action', { action: 'set.clear-override', params });
            } else {
                this.$dispatch('progression-action', { action: 'set.override', params });
            }
        },
        ...
        startEdit(rowIndex, colIndex) {
            ...
            const currentValue = this.getCellValue(rowIndex, colIndex);
        ...
        commitEdit() {
            ...
            if (value !== normalizedOriginal) {
                this.dispatchOverride(this.editingCell.row, this.editingCell.col, value === '' ? null : value);
            }
