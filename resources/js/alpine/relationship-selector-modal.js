document.addEventListener('alpine:init', () => {
    ...
    Alpine.data('relationship_selector_modal', (config = {}) => ({
        ...
        defaultLoaderListKey() {
            return this.lists.find((list) => list?.loader?.type === 'default')?.key ?? null;
        },
        ...
        async runAction(action, listKey, row) {
            ...
            if (action.type === 'wire') {
                await this.invokeWireAction(action.name, listKey, row);
        ...
        async invokeWireAction(method, listKey, row) {
